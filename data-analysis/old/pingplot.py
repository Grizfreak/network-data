...
def plot(data, events, debug=False, fig_size=(24, 8)):
    ...
    ping_values = pd.to_numeric(data["Ping (ns)"], errors="coerce")
    plot_data = pd.DataFrame({"x": x_values, "Ping (ns)": ping_values}).dropna(subset=["x", "Ping (ns)"])
    ax.plot(plot_data["x"], plot_data["Ping (ns)"], label="Ping")
    ax.set_xlabel(x_label)
    ax.set_ylabel("Ping (ns)")
    fig.suptitle("Ping over Time" if is_time_axis else "Ping over Frames")
